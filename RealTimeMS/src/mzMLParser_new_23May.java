...
public class mzMLParser_new_23May
{
	...
	public static void main(String[] args) 
	{
		...
		*/
		...
		//DBseqID=PepEvid
		Hashtable<String, String> print_DBseqID_to_PepEvid = map_values.getDBseqID_to_PepEvid();
		...
		ArrayList<IDs> MappedArr = new ArrayList<IDs>();
		
		//System.out.println("Printing the spectra attributes: ");
		
		//Hash set to store the peptide evidences that map to the same dBSequence
		Set<String> double_dBSeq = new HashSet<String>();
		...
		for(int i = 0; i < Arr.size(); i++)
		{
			...
			if(map_values.getScanT_to_ScanNum().containsKey(startTime))
			{
				...
				MappedSpec = new IDs(startTime, scanNum, pepEv, pepRef, dBSeq, pepSeq, acc);
		...
		//---------------------Checking for multiple peptides---------------------
		for(int j = 0; j < MappedArr.size(); j++)
		{
			//Go through each spectrum contained in the arraylist
			if(map_values.getPepEvid_to_DBseqID().contains(MappedArr.get(j).getdBSequence_id()))
			{
				
			}
			
			/*
			//If the Hashtable of peptide evidence to dBSequences already contains that dBSequence
			if(map_values.getPepEvid_to_DBseqID().contains(MappedSpec.getdBSequence_id()))
			{
				//Get the peptide evidence associated with that duplicate dBSequence
				map_values.getPepEvid_to_DBseqID().get
			}
			*/
		...
		//---------------------Function to associate accession---------------------
		//creates an arraylist for each accession where each cell contains a specific time, # of identifications, and the associated peptide evidences 
